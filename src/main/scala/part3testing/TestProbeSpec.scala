...
class TestProbeSpec extends TestKit(ActorSystem("TestProbeSpec")) with ImplicitSender with AnyWordSpecLike with BeforeAndAfterAll {
  ...
  "A master actor" should {
    ...
    "send the work to the slave actor" in {
      ...
      slave.expectMsg(SlaveWork(workLoadString, testActor)) // Remember testActor is the default injected actor who sends messages to Master Actor
      slave.reply(WorkCompleted(3, testActor))

      expectMsg(Report(3)) // TestActor receives Report(3)
    }
    ...
    "aggregate data correctly" in {
      ...
      slave.receiveWhile() {
        // `` guaranties it receives exactly the concept between ``
